...
def mainModel(country, validation):
    total_data = {}
    ...
    for key, value in rawgeomap.items():
        geomap[key.split(":")[0].lower()] = value+random.uniform(-vari, vari)
    ...
    for key, value in rawpopularity.items():
        popularity[key.split(":")[0].lower()] = value+random.uniform(-vari, vari)
    ...
    for key, value in sustainability.items():
        total_data[key.lower()]["C3"] = value+random.uniform(-vari, vari)
    ...
    with open("Model/246.csv", "r") as f:
        ...
        for row in reader:
            a = float(row[4])
            b = float(row[5])
            gender[row[0].lower()] = 1-abs((a-b)/(a+b))+random.uniform(-vari, vari)
            total_data[row[0].lower()]["C2"] = gender[row[0].lower()]
            

            inclusivity[row[0]] = float(row[-1])+random.uniform(-vari, vari)#*random.uniform(0.5, 2)

            injury[row[0]] = float(row[1])+random.uniform(-vari, vari)#*random.uniform(0.5, 2)
    ...
    for key, value in sorted_data.items():
        trunked_data[key] = value["Final"]
    return trunked_data

with open("Model/1-1 Local.json", "r") as f:
    local = json.load(f)
    countries = list(local.keys())
    disciplines = list(local["United States"].keys())

ourModel = {}
for country in countries:
    with open("Model/output for " + country + ".json", "r") as f:
        sorted_data = json.load(f)
        trunked_data = {}
        for key, value in sorted_data.items():
            trunked_data[key] = value["Final"]
        ourModel[country] = trunked_data

validationset = ["breaking", "baseball", "flag football", "fencing", "rowing", "artistic gymnastics", "esports", "cricket", "ultimate frisbee"]

def wholeModel(seeed):
    #random.seed(seeed)
    #validationset = random.sample(disciplines, 9)
    total_score = 0
    for country in countries:
        diff_score = 0
        data = mainModel(country, validationset)


        for item in ourModel[country].keys():
            diff_score += (ourModel[country][item] - data[item])**2
        
        total_score += (diff_score / len(disciplines)) ** 0.5
    return total_score / len(countries)
